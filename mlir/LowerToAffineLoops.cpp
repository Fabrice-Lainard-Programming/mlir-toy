...
{
  return std::max_element(llvm::adl_begin(Range), llvm::adl_end(Range), C);
}
...
namespace
{
  ...
  struct BinaryOpLowering : public ConversionPattern
  {
    ...
    // Verify the types of operands
    auto lhsType = lhs.getType().dyn_cast<FloatType>();
    auto rhsType = rhs.getType().dyn_cast<FloatType>();

    if (!lhsType || !rhsType) return failure();

    // Replace with arith::AddFOp for floating-point addition
...
void ToyToAffineLoweringPass::runOnOperation()
{
  ...
  target.addLegalOp<toy::PrintOp>();
   target.addLegalOp<toy::ConstantOp>();

  // Now that the conversion target has been defined, we just need to provide
  // the set of patterns that will lower the Toy operations.
  RewritePatternSet patterns(&getContext());
  patterns.add<AddOpLowering, FuncOpLowering, MulOpLowering,
               PrintOpLowering, ReturnOpLowering>(
      &getContext());
