...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    // Generate a call to printf for the double value.
    auto printOp = cast<toy::PrintOp>(op);
    auto doubleValue = printOp.getInput();
    rewriter.create<LLVM::CallOp>(
        loc, getPrintfType(context), printfRef,
        ArrayRef<Value>({formatSpecifierCst, doubleValue}));
...
struct ConstantIntOpLowering : public ConversionPattern {
  explicit ConstantIntOpLowering(MLIRContext *context)
  ...
  LogicalResult matchAndRewrite(
    ...
    Type llvmF64Type =  rewriter.getF64Type(); 
